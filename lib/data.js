...
/**
 * List all items (files) in a directory
 * @param {string} dir - The directory name (inside .data)
 * @param {function} callback - The callback function to return error or file list
 */
lib.list = (dir, callback) => {
  ...
  fs.readdir(dirPath, (err, files) => {
    if (err) {
      return callback(`Error reading directory: ${err.message}`);
    }

    if (files.length <= 0) {
      return callback("Error: Directory is empty.");
    }

    // Filter out non-.json files and strip the .json extension from file names
    const trimmedFileNames = files
      .filter((file) => file.endsWith(".json"))
      .map((file) => file.replace(".json", ""));

    callback(null, trimmedFileNames); // Success, return the list of files
  });
};
